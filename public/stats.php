...
<body>
	...
	<?php 
		...
		if($log->num_rows != 0) {
			...
			while($row = $log->fetch_assoc()) {
				...
				$profit = number_format(calculate_profit_for_record($row), 2, '.', ',');
				...
				$string .= '<td id="td_border">' . $profit . '</td>';
	...
	Highest profit: <?php echo isset($max_profit)?$max_profit:'Var unset'; ?><br>
	Average profit: <?php echo isset($avg_profit)?$avg_profit:'Var unset'; ?><br>
	Total profit: <?php echo isset($total_profit)?$total_profit:'Var unset'; ?><br>
	<br>
